...
def delete_files_and_dir(dir) -> None:
...
def make_copy_dataset_random(name: str):
    ...
    for file in enumerate(all_good_files):
        try:
            os.rename('./dataset/good/copy/' +
                      file[1], './' + name + '/good_'+file[1])
    ...
    for file in enumerate(all_bad_files):
        try:
            os.rename('./dataset/bad/copy/' +
                      file[1], './' + name + '/bad_'+file[1])
        except Exception as e:
            continue
        # end try
            ...
            if count == 0:
                ...
                while 1:
                    ...
                    if not (rand_num in rand_names):
                        os.rename(row[1], row[1][0:row[1].rfind(
                            "\\") + 1]+str(rand_num)+".txt")
                        rand_names.append(rand_num)
                        row[0] = row[0][0:row[0].rfind(
                            "\\") + 1] + str(rand_num)+".txt"
                        row[1] = row[1][0:row[1].rfind(
                            "\\") + 1] + str(rand_num)+".txt"
                        list_objects.append(row)
                        break
            count += 1
